...
print(
    f"  Unlabeled years: {unlabeled['ProxySeason'].min()}-{unlabeled['ProxySeason'].max()}"
)
...
for feat in key_features:
    if feat in df.columns:
        print(f"\n{feat}:")
        print(
            f"  Labeled:   {labeled[feat].mean():.4f} (std: {labeled[feat].std():.4f})"
        )
        print(
            f"  Unlabeled: {unlabeled[feat].mean():.4f} (std: {unlabeled[feat].std():.4f})"
        )

        # Check missing rates
        print(
            f"  Missing - Labeled: {labeled[feat].isna().mean():.1%}, Unlabeled: {unlabeled[feat].isna().mean():.1%}"
        )
...
for rat in rationales:
    ...
    print(
        f"  {rat}: {pos_rate:.1%} ({labeled[rat].sum():.0f} / {labeled[rat].notna().sum():.0f})"
    )
