...
def main():
    ...
    parser.add_argument(
        "--max_iterations", type=int, default=5, help="Maximum self-training iterations"
    )
    parser.add_argument(
        "--min_pseudo_labels",
        type=int,
    ...
    parser.add_argument(
        "--no_calibrate", action="store_true", help="Disable probability calibration"
    )
    ...
    print(f"\nTo evaluate, run:")
    print(
        f"  python scripts/evaluate.py --model_dir models/semi_supervised_{args.method}"
    )
    print(f"\nTo generate predictions, run:")
    print(
        f"  python scripts/predict.py --model_dir models/semi_supervised_{args.method}"
    )
    print(f"{'=' * 80}\n")
