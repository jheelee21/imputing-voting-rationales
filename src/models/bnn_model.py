    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            ...
            layer.weight = PyroSample(
                dist.Normal(0., weight_scale).expand([layer_dims[i + 1], layer_dims[i]]).to_event(2)
            )
...
class BNNModel(BaseRationaleModel):
    ...
    def fit(
        ...
        if verbose:
            final_loss = self.validation_losses[-1] if self.validation_losses else self.training_losses[-1]
    ...
    def predict_proba(
        ...
        with torch.no_grad():
            for _ in range(num_samples):
                ...
                guide_trace = pyro.poutine.trace(self.guide).get_trace(x=X_tensor, y=None)
        ...
        all_probs = np.array(all_probs)  # Shape: (num_samples, batch_size, num_rationales)
    ...
    def load(cls, filepath: str, device: str = None):
        ...
        if save_dict.get('param_store'):
            ...
            print(f"BNN model loaded from {filepath} ({input_dim} features → {output_dim} outputs)")
        else:
