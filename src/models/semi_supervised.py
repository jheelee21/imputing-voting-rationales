    ...
    def fit(
        ...
        for iteration in range(self.max_iterations):
            ...
            if n_pseudo < self.min_pseudo_labels:
                if verbose:
                    print(f"Only {n_pseudo} pseudo-labels (< {self.min_pseudo_labels}). Stopping.")
                break
    ...
    def fit(
        ...
        for iteration in range(self.max_iterations):
            ...
            self.model1 = SupervisedRationaleModel(
                self.rationale, self.base_model_type, self.calibrate,
                self.custom_params, self.random_seed
            )
...
class MultiLabelSemiSupervised:
    ...
    def fit(
        ...
        for i, rationale in enumerate(self.rationales):
            ...
            model.fit(
                X_labeled, y_single, X_unlabeled,
                X_val, y_val_single, verbose=verbose
            )
